...
public class LogWrite {
		...
		synchronized (oLockObject) {
			...
			if (mm == null || !dateString.substring(0, 10).equals(logTitle)) {
				...
				try {
					...
					mm = new RandomAccessFile(InitParamters.logPath + logTitle+"_log.txt", "rw");
		...
		synchronized (oLockObject2) {
			...
			if (mm2 == null || !dateString.substring(0, 10).equals(logTitle2)) {
				...
				try {
					...
					mm2 = new RandomAccessFile(InitParamters.logPath + logTitle2 + "_err.txt", "rw");
	...
	public static void println(String pos, Exception e) {
		synchronized (oLockObject2) {
			Date currentTime = new Date();
			SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss ");
			String dateString = formatter.format(currentTime);
